...
You should see the server start on `http://localhost:5001`.
...
### List check-ins
```bash
curl http://localhost:5001/api/checkins
```

### Credits balance + ledger
```bash
curl http://localhost:5001/api/credits/balance
curl http://localhost:5001/api/credits/ledger
```
...
```bash
curl -X POST http://localhost:5001/api/events/blackboard ^
  -H "Content-Type: application/json" ^
...
```bash
curl http://localhost:5001/api/insights?days=7
```
