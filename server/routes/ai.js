...
router.post('/analyze', async (req, res) => {
  try {
    ...
    let prompt = `You are a friendly and encouraging health assistant. The user just submitted their daily health check-in. `;
    
    if (checkInData.transcript) {
      prompt += `The user also left a voice note: "${checkInData.transcript}". Please respond directly and empathetically to their voice note, and offer a brief, 2-3 sentence insight based on it.`;
    } else {
      prompt += `
Here is the data:
